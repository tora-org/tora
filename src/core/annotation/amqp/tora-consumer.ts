...
import { ConsumerFunction, DecoratorClass, ToraConsumerOptions } from '../__types__'
...
export function ToraConsumer(options?: ToraConsumerOptions): DecoratorClass {
    return constructor => {
        ...
        meta.set({
            ...
            on_load: (meta, injector) => load_component(constructor, injector, meta, 'œœ-ToraConsumer'),
            function_collector: () => {
                const touched = set_touched(constructor).value
                return Object.values(touched)
                    .filter((item): item is ConsumerFunction<any> => item.type === 'ToraConsumerFunction')
            }
        })
    }
}
